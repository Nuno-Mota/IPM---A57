...
function drawTextMarkers(options) {
	...
	for (iTick = 10; iTick < 180; iTick += 20) {
		...
		innerTickX = gaugeOptions.radius - (Math.cos(degToRad(iTick)) * (gaugeOptions.radius + 10)) - 5;
		innerTickY = gaugeOptions.radius - (Math.sin(degToRad(iTick)) * (gaugeOptions.radius + 10));
		...
		// MPH increase by 0.2 every 20 degrees
		iTickToPrint += 0.2;
		iTickToPrint = iTickToPrint.toFixed(2);
		iTickToPrint = parseFloat(iTickToPrint);
...
function drawSpeedometerColourArc(options) {
	...
	drawSpeedometerPart(options, 0.9, "rgb(249, 140, 0)", endOfGreen);
	...
	*/
	...
	if(bDecrement) {
		if(iCurrentSpeed - 10 < iTargetSpeed)
			iCurrentSpeed = iCurrentSpeed - 0.01;
		else
			iCurrentSpeed = iCurrentSpeed - 0.075;
	} else {
...
function drawResults(){
	var iSpeedAsAngle = txtSpeed*0.9/45;
	iSpeedAsAngle = iSpeedAsAngle.toFixed(2);
	iSpeedAsAngle = parseFloat(iSpeedAsAngle);
	document.getElementById('textInput').value=iSpeedAsAngle;
	if(iSpeedAsAngle <= 0.49)
		document.getElementById("textInput").style = "color: green";
	if(iSpeedAsAngle >= 1.2)
...
function drawWithInputValue() {
	...
	txtSpeed = Math.floor((Math.random() * 79) + 1);
	...
	if (txtSpeed !== null) {
		...
		setTimeout("drawResults()", 7500);
		setTimeout("drawReset()", 20000);
    }
}
